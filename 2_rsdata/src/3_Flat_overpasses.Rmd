...
# unq.inv <- read_feather('output/unique_site_inventory.feather') %>%
#   arrange(SiteID) %>%
#   filter(!is.na(source)) %>%
#   select(-lat,-long)
...
#wqp.lagos <- read_feather('output/wqp_usgs_unity_stream.feather')
#wqp.lagos.usgs.unity <- read_feather('output/wqp_lagos_usgs_unity.feather')
...
# dups <- site.candidates %>%
#   duplicated(  by=c("SiteID", "source", "date"))
...
# I changed to site.candidates.utc and timediff to date_utc not date_unity
...
#data_file <- 'output/wide_pull.feather'
#write_feather(wqp.pull,path=data_file)


#data_file <- '/2_rsdata/out/wide_pull_v3.feather'
