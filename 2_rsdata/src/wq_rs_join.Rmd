...
#wide.pull.old <- read_feather("D:/Dropbox/projects/TSS/output/wide_pull_v2.feather")

#wide.pull.old2<- read_feather("D:/Dropbox/projects/TSS/output/wide_pull.feather")

#wide.pull.old2  %>% group_by(source) %>% summarise(count= n())
...
#hist(as.numeric(sr.type$timediff))
##
...
# conc_long %>% filter(harmonized_parameter=="tss") %>% summarise(methods =unique(analytical_method))
# 
# unique(conc_long$analytical_method)
# full.inv <- full.inv.raw %>%
...
#test %>% group_by(characteristicName) %>% summarise(n=n())

### there is missing data beacuse need to make long_with_methods with new usgs
...
# this datasat has some duplicates, remove for each consituent
...
#Setup save path
#data_file <- 'output/sr_wq_rs_join_test.csv'

#Save it locally as a csv file
#write_csv(sr.type,path=data_file)

test <-sr.clean.long %>% filter(parameter == "tss") %>%
  filter(value >1, red < 10000, red >1) # %>%
  #filter(!characteristicName %in% c("Suspended sediment concentration (SSC)","Suspended Sediment Concentration (SSC)" ))
  
ggplot(data= test, aes(x=value, y=green/nir)) + geom_point() +
  scale_x_log10()
